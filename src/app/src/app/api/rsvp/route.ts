...
// GET /api/rsvp -> shows env status and Guest row count
export async function GET(req: Request) {
  try {
    ...
    if (error) return NextResponse.json({ ok: false, step: "select", error: error.message }, { status: 500 });
...
// POST /api/rsvp -> inserts a guest row
